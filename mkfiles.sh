...
sed -i ${ROOT}/configuration.yaml \
    ...
    -e 's/LastKnownBlockVersion-Major: 0/LastKnownBlockVersion-Major: 4/' \
...
for N in ${BFT_NODES_N}; do
  ...
  cardano-cli byron key keygen --byron-formats \
...
sed -i shelley/genesis.spec.json \
    -e 's/"slotLength": 1/"slotLength": 0.5/' \
    -e 's/"activeSlotsCoeff": 5.0e-2/"activeSlotsCoeff": 0.5/' \
